document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DISPLAY ALL TORRENT DETAILS (THIS FUNCTION IS FIXED) ---
    const displayTorrentDetails = (torrent) => {
        // Update page title
        if (pageTitle) pageTitle.textContent = `${torrent.name} - The Pirate Bay`;
        
        // Populate Details Info Box
        if (detailsTitle) detailsTitle.textContent = torrent.name;

        // --- THE FIX IS HERE ---
        // We use optional chaining (?.) to safely access the username.
        // If torrent.uploader is null, it will gracefully result in 'Anonymous'.
        const uploaderUsername = torrent.uploader?.username || 'Anonymous';
        const uploaderLink = torrent.uploader 
            ? `<a href="/profile?user=${uploaderUsername}">${uploaderUsername}</a>` 
            : uploaderUsername; // If no uploader, just display the text 'Anonymous' without a link.
        ...
        if (infoHashSpan) infoHashSpan.textContent = torrent.infoHash.toUpperCase();
        
        // Populate Description
        if (descriptionPre) descriptionPre.textContent = torrent.description;

        // Populate File List
        if (torrent.files && torrent.files.length > 0) {
            if (fileListTitle) fileListTitle.textContent = `Files (${torrent.files.length})`;
            let fileListHTML = '';
            torrent.files.forEach(file => {
                fileListHTML += `<div><span>${file.path}</span><span>${formatBytes(file.size)}</span></div>`;
            });
            if (fileListFrame) fileListFrame.innerHTML = fileListHTML;
        } else {
            if (fileListFrame) fileListFrame.innerHTML = '<div>No file information available.</div>';
        }
        ...
        if (loggedInUser) {
            ...
            if (bookmarkBtnContainer) bookmarkBtnContainer.innerHTML = '<p><a href="/login">Log in</a> to bookmark this torrent.</p>';
        }
    };

    // --- BOOKMARK BUTTON RENDER & LOGIC (SIMPLE TEXT VERSION) ---
    const renderBookmarkButton = (torrentId, isBookmarked) => {
        if (!bookmarkBtnContainer) return;
